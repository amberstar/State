...
| -------------------------------- | -------------------------- | :----------------:|
| `STState.Enum` | Specify entity is an enum. All attributes are considered cases of the enum. Specify raw value type with `STState.Type` . *required for enums. | `YES`, `NO` |
| `STState.Protocol` | Specify entity is a protocol. (**note:** you can also check the abstract checkbox) | `YES`, `NO` |
| `STState.Type`       | When used on `Enum` entities, declares the enum as a raw value enum, and specifies it's raw value type. When used on `Protocol` entities specifies protocol inheritance.  |**For Enums:** The exact type for `Enum` **-or**  **For Protocols:** comma seperated list of protocols to inherit  |
| `STState.Model`       | Used for raw value enums, and specifies it's raw value type as a model type.  |`Model`, `ModelArray`, `ModelDictionary`  |
| `STState.Transformable`       | Used for raw value enums, and specifies it's raw value type as transformable.  |The exact name of the transform type to use. |
...
| Key                              | Description                | Values            |
| -------------------------------- | -------------------------- | :----------------:|
| `STState.Immutable`    | Specify an attribute is immutable (code will use `let` instead of `var` for the property) | `YES`, `NO` |
| `STState.Type`       | Specify a type for the attribute. For example, `[String : Int]` would specify to use a Dictionary of String, Ints for this property **-or-** Specifies the associated value type for enum cases.| The exact type  |
| `STState.Func`       | Used on `STState.Protocol` entities with a requirement type of `func` specifies the function signature for the requirement| The exact signature of the function requirement  |
| `STState.Mutating`       | Used on `STState.Protocol` entities with a requirement type of `func`  specifies the function is `mutating` | `YES`, `NO`  |
| `STState.ProtocolRequirementType`       | Used on STState.Protocol entities. Specifies attribute requirment type for the protocol. **note: if not specified `var` is assumed, use `get` to generate a get only variable requirement| `func`, `var`, `get`  |
...
## Default Values:
You can specify a default value in the inspector window, or use `STState.Value` in the user info section to give a property a default value. **Note:** because of swift 1.2 bugs, default values are not fully supported. Current limitations are:
...
- If you enter a custom type using `STState.Type` key, it will override the type in the attribute type option list.
...
- Even if you specify a type with `STState.Type` you have to still choose a type in the type section box. (it will be ignored) If you don't, the project will not build.
