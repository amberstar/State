...
class DataAdapterTests: Test {
    ...
    func testReadingJSONAndWritingPlist() {
        let users_out : UserTypes? = Decoder.decode(jsonData)
        Plist.write(Encoder.encode(users_out!), path: tempPathFor("users.plist"))
        let users : UserTypes? = Decoder.decode(Plist.read(tempPathFor("users.plist")))
        XCTAssert(users != nil)
